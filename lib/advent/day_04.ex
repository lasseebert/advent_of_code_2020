defmodule Advent.Day04 do
  ...
  defp valid?(passport) do
    all_keys?(passport) and passport |> Map.keys() |> Enum.all?(&valid_field?(&1, Map.fetch!(passport, &1)))
  end
  ...
  defp valid_field?("hgt", value) do
    Regex.run(~r/^(\d+)(cm|in)$/, value, capture: :all_but_first)
    |> case do
  ...
  # cid (Country ID) - ignored, missing or not.
  defp valid_field?("cid", _value) do
    true
  end

  defp all_keys?(passport) do
